-- Joao mm2 V4 - Nearest Coin + Ops + Kill All
getgenv().SecureMode = false
local Rayfield = loadstring(game:HttpGet("https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/main/source.lua"))()
local Window = Rayfield:CreateWindow({
   Name = "Joao mm2",
   LoadingTitle = "Joao mm2 V4",
   LoadingSubtitle = "Fix kick + Fling + Kill All",
...
local function TweenTo(pos, altura)
    ...
    -- anti kick invalid position
    if not pos or pos.Y < -30 or pos.Magnitude > 10000 then return end
    local dist = (HRP.Position - pos).Magnitude
    local tempo = dist / Speed
    if tempo < 0.15 then tempo = 0.15 end
    if tempo > 3 then tempo = 3 end
...
local function GetSortedCoins()
    ...
    for _,coin in pairs(CC:GetChildren()) do
        if coin.Name=="Coin_Server" and coin:IsA("BasePart") and coin.Parent and coin.Position.Y > -20 then
            local d = (HRP.Position - coin.Position).Magnitude
            if d < 250 then -- só pega as perto pra não cruzar o mapa todo
        ...
        if AutoCoin then
            pcall(function()
                ...
                for _,data in ipairs(coins) do
                    ...
                    if data.c and data.c.Parent then
                        TweenTo(data.c.Position, 3)
                        task.wait(0.08)
                    end
                end
            end)
...
local function FlingTarget(target)
    ...
    -- metodo fling girando
    for i=1,15 do
        HRP.CFrame = THRP.CFrame * CFrame.new(0,0,0.3)
...
OpsTab:CreateButton({ Name="Fling Xerife", Callback=function()
    for _,p in pairs(Players:GetPlayers()) do if GetRole(p)=="Sheriff" then FlingTarget(p) break end end
end, })
OpsTab:CreateButton({ Name="Fling Assassino", Callback=function()
    for _,p in pairs(Players:GetPlayers()) do if GetRole(p)=="Murderer" then FlingTarget(p) break end end
end, })
...
local function CreateSmartButton()
    ...
    Btn.MouseButton1Click:Connect(function()
        pcall(function()
            ...
            for _,r in pairs(game.ReplicatedStorage:GetDescendants()) do if r:IsA("RemoteEvent") and r.Name:lower():find("shoot") then pcall(function() r:FireServer(target) end) end end
            gun:Activate()
        end)
    end)
end
...
CombatTab:CreateButton({ Name="Tween Arma e Volta", Callback=function()
    task.spawn(function()
        local HRP=LP.Character.HumanoidRootPart local Old=HRP.CFrame Bringing=true
        for _,o in pairs(workspace:GetDescendants()) do if o.Name=="GunDrop" and o:IsA("BasePart") then TweenTo(o.Position,2) task.wait(0.3) TweenTo(Old.Position,2) break end end
        Bringing=false
    end)
end, })

CombatTab:CreateButton({ Name="Matar Todos [Assassino]", Callback=function()
    if GetRole(LP)~="Murderer" then return end
    task.spawn(function()
        Bringing=true
        local knife = LP.Character:FindFirstChild("Knife") or LP.Backpack:FindFirstChild("Knife")
        if knife then LP.Character.Humanoid:EquipTool(knife) end
        for _,p in pairs(Players:GetPlayers()) do
            if p~=LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                p.Character.HumanoidRootPart.CFrame = LP.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,-2)
                task.wait(0.25)
            end
        end
        Bringing=false
